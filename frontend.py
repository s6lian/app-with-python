...
def view_command():
    ...
    for row in backend.view():
        list1.insert(END,row) # every row insert into the end of listbox
...
def add_command():
    ...
    year_text.get(), isbn_text.get())
    list1.delete(0,END)
    list1.insert(END,"Successfully insert into bookstore!"+str((title_text.get(), author_text.get(),
    year_text.get(), isbn_text.get())))
...
list1.bind('<<ListboxSelect>>',get_selected_row) #bind the event with the function
